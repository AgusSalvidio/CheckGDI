...
def _drawMetalBezel(canvas: tk.Canvas, cx: float, cy: float, outerRadius: float, innerRadius: float,
    ...
    for i in range(rings):
        ...
        color = "#%02x%02x%02x" % (
            int(lr + (dr - lr) * t), int(lg + (dg - lg) * t), int(lb + (db - lb) * t),
        )
...
def _drawTicksAndNumbers(canvas: tk.Canvas, cx: float, cy: float, faceRadius: float, gdiMax: int,
    ...
    while tick <= gdiMax:
        ...
        outer = _point(cx, cy, faceRadius * 0.80, angle)
        inner = _point(cx, cy, faceRadius * 0.68, angle)
        canvas.create_line(*outer, *inner, width=majorWidth, fill=color)

        labelPoint = _point(cx, cy, faceRadius * 0.52, angle)
        canvas.create_text(
        ...
        if tick < gdiMax:
            for m in range(1, minorDivisions):
                ...
                mOuter = _point(cx, cy, faceRadius * 0.80, mAngle)
                mInner = _point(cx, cy, faceRadius * 0.74, mAngle)
...
def _drawNeedleSport(canvas: tk.Canvas, cx: float, cy: float, angleDeg: float, length: float,
    ...
    canvas.create_polygon(
        ...
        fill=GAUGE_NEEDLE_ACCENT, outline="",
    )
...
def _drawNeedle(canvas: tk.Canvas, cx: float, cy: float, angleDeg: float, length: float,
                 style: str, needleColor: str, tailColor: str, thickness: float = 1.0) -> None:
...
ANALOG_GAUGE_SPECS: dict[str, AnalogGaugeSpec] = {
    "classic": AnalogGaugeSpec(
        ...
        faceRadiusRatio=0.84, faceColor=GAUGE_FACE, innerRingColor=None,
        tickColor=GAUGE_TICK, numberColor=GAUGE_NUMBER, fontFamily="Consolas", thinTicks=False,
        needleColor=GAUGE_NEEDLE, needleTailColor=GAUGE_NEEDLE_TAIL, needleLengthRatio=0.79,
    ...
    "retro": AnalogGaugeSpec(
        ...
        faceRadiusRatio=0.90, faceColor=GAUGE_RETRO_FACE, innerRingColor=GAUGE_RETRO_TICK,
        tickColor=GAUGE_RETRO_TICK, numberColor=GAUGE_RETRO_NUMBER, fontFamily="Georgia", thinTicks=True,
        needleColor=GAUGE_RETRO_NEEDLE, needleTailColor=GAUGE_RETRO_TICK, needleLengthRatio=0.80,
...
def _drawAnalog(canvas: tk.Canvas, cx: float, cy: float, radius: float, count: int, gdiMax: int,
                 ...
                 needleThickness: float, spec: AnalogGaugeSpec) -> None:
    """Shared renderer for every analog (needle-based) gauge style, parametrized by `spec`."""
    _drawMetalBezel(canvas, cx, cy, radius, radius * 0.90, spec.bezelLight, spec.bezelDark, rings=spec.bezelRings)
    ...
    _drawTicksAndNumbers(
        canvas, cx, cy, faceRadius, gdiMax, spec.tickColor, effectiveNumber, spec.fontFamily, thin=spec.thinTicks,
    ...
    canvas.create_text(
        ...
        fill=effectiveNumber, font=(spec.fontFamily, max(int(faceRadius * spec.labelFontRatio), 7), "bold"), anchor="center",
    )

    pct = min(count / gdiMax, 1.0)
    _drawNeedle(
        canvas, cx, cy, _angleFor(pct), faceRadius * spec.needleLengthRatio, needleStyle,
        effectiveNeedle, spec.needleTailColor, needleThickness,
    )
    ...
    canvas.create_text(
        ...
        text=str(count),
        fill=effectiveNumber, font=(spec.fontFamily, max(int(faceRadius * spec.numberFontRatio), 8), "bold"), anchor="center",
    )


def _drawDigital(canvas: tk.Canvas, cx: float, cy: float, radius: float, count: int, gdiMax: int,
                   warnThreshold: int, criticalThreshold: int, dialColor: str) -> None:
    ...
    canvas.create_text(
        cx, cy - radius * 0.18,
        text="GDI",
        fill=GAUGE_DIGITAL_LABEL, font=("Consolas", max(int(radius * 0.09), 7), "bold"), anchor="center",
    )
    canvas.create_text(
        cx, cy + radius * 0.08,
        text=str(count),
        fill=color, font=("Consolas", max(int(radius * 0.32), 10), "bold"), anchor="center",
    )
    canvas.create_text(
        cx, cy + radius * 0.38,
        text=f"x1000  ·  {count / 1000:.1f}k / {gdiMax // 1000}k",
        fill=GAUGE_DIGITAL_LABEL, font=("Consolas", max(int(radius * 0.07), 7)), anchor="center",
    )
...
def draw(canvas: tk.Canvas, width: int, height: int, count: int, gdiMax: int,
          ...
          numberColor: str = "auto", dialColor: str = "auto", needleThickness: float = 1.0) -> None:
    ...
    if gaugeStyle == "digital":
        _drawDigital(canvas, cx, cy, radius, count, gdiMax, warnThreshold, criticalThreshold, dialColor)
    else:
        spec = _analogSpec(gaugeStyle)
        _drawAnalog(
            canvas, cx, cy, radius, count, gdiMax, warnThreshold, criticalThreshold, needleStyle,
            needleColor, zoneMode, accentColor, numberColor, dialColor, needleThickness, spec,
        )
